...
if(!$mysqli){  
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST')
  {
      ...
      $sql2 = "UPDATE `acc_ase`,`orders`  SET `acc_ase`.`dept_due` = `acc_ase`.`dept_due`+`orders`.`total_price` FROM `orders` JOIN `acc_ase` WHERE `orders`.`roll`=`acc_ase`.`roll` AND `orders`.`order_status`='Cancelled';";
      ...
      echo "<script type='text/javascript'>alert('$message');</script>";
      $sql4 ="UPDATE `php_users_login` SET `acc_status`='0' FROM `acc_ase` WHERE `dept_due`>='200';";
      if(mysqli_query($mysqli, $sql3))
      {  
          $message = "ACC STATUS updated";
      }
      else
      {  
          $message = "Could not insert record"; 
